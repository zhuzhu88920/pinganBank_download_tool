...
(function() {
    ...
    let currentCompany = 'wj', currentTab = 0, qPressed = false, companyKey = '';
    ...
    async function queryMonthly(ym) {
        const c = getCompany();
        return (await request(API.queryMonthly, { accountName: c.accountName, accountNo: c.accountNo, startDate: ym, endDate: ym, turnPageBeginPos: 1, turnPageShowNum: 10 }))?.data?.statementList?.[0];
    }

    async function downloadMonthly(ym) {
        const c = getCompany(), s = await queryMonthly(ym);
        if (!s?.pdfUdmpDocId) return false;
        return download(await request(API.downloadMonthly, { accountName: c.accountName, accountNo: c.accountNo, startDate: ym, endDate: ym, turnPageBeginPos: 1, turnPageShowNum: 10, udmpDocId: s.pdfUdmpDocId }, true), `${ym.slice(0,4)}_${ym.slice(4,6)}_yjd_${currentCompany}.pdf`);
    }

    async function queryTrans(s, e) {
        const c = getCompany();
        return parseInt((await request(API.queryTrans, { accountNo: c.accountNo, accountName: c.accountName, accountBankName: c.accountBankName || '', ccy: 'RMB', tranType: '', counterAcctNo: '', counterAcctName: '', startDate: s, endDate: e, ranking: '0', turnPageBeginPos: 1, turnPageShowNum: 10, queryType: '02', accountType: '0' }))?.data?.turnPageTotalNum || 0);
    }

    async function downloadTrans(n, s, e, name) {
        return download(await request(API.downloadTrans, { accountNo: getCompany().accountNo, accountName: getCompany().accountName, accountBankName: getCompany().accountBankName || '', ccy: 'RMB', tranType: '', counterAcctNo: '', counterAcctName: '', startDate: s, endDate: e, ranking: '0', turnPageBeginPos: 1, turnPageShowNum: n, queryType: '02', accountType: '0', fileType: '01', turnPageTotalNum: n }, true, { 'adtoken': await getAntiToken() }), name);
    }

    function renderMonthBtns(pIdx) {
    ...
    function createUI() {
        ...
        div.innerHTML = `
            <div style="margin-bottom:10px;"><label style="display:block;margin-bottom:5px;">公司:</label><select id="c" style="width:100%;padding:6px;border:1px solid #ddd;border-radius:4px;">${Object.entries(COMPANIES).map(([k,v])=>`<option value="${k}" ${k===currentCompany?'selected':''}>${v.name}</option>`).join('')}</select></div>
            ...
            <div style="margin-bottom:10px;font-size:10px;color:#999;">F1-回单 F2-月结单 F3-明细 | 1-9,0-月份 | Q+1~4-季度 | A-全选 | C-清空 | W/J-网聚 B/F-爆发 H/F-公司 | Enter-下载</div>
        ...
        const panels = [div.querySelector('#m'), div.querySelector('#m'), div.querySelector('#m')];
        const yearSel = div.querySelector('#y');
        const btn = div.querySelector('#b'), msg = div.querySelector('#msg');

        function updateMonthBtns() {
            panels[0].innerHTML = renderMonthBtns(0);
            document.querySelectorAll('#m span').forEach(s => s.onclick = function() {
        ...
        document.onkeydown = function(e) {
            ...
            else if (k === 'W') { companyKey = 'W'; }
            else if (k === 'B' && companyKey === '') { companyKey = 'B'; }
            else if (k === 'H' && companyKey === '') { companyKey = 'H'; }
            else if (k === 'J' && companyKey === 'W') { div.querySelector('#c').value = 'wj'; currentCompany = 'wj'; companyKey = ''; log('切换: 网聚', '#ff6600'); }
            else if (k === 'F' && (companyKey === 'B' || companyKey === 'H')) { div.querySelector('#c').value = companyKey === 'B' ? 'bf' : 'hff'; currentCompany = companyKey === 'B' ? 'bf' : 'hff'; companyKey = ''; log(`切换: ${currentCompany === 'bf' ? '爆发' : '昊峰坊'}`, '#ff6600'); }
            else companyKey = '';
        };
